...
//        İşleme başlarken bir önceki değer olmadığı için bu değer identity parametresinde tanımlanmaktadır.
    ...
    public static void elemanlariCarp(List<Integer> sayilar){
        ...
        System.out.println("sayilar.stream().reduce(1,(a,b)-> a*b) = " +
                sayilar.stream()
                        .filter(Methodlarim::ciftElamaniBul)//method ref kullanildi kendi class imizda
                        // create ettigimiz seed methodu kulandik
                        .reduce(1, (a, b) -> a * b));
        //method referans ile
        System.out.println(sayilar.stream().filter(Methodlarim::ciftElamaniBul)
                .reduce(Math::multiplyExact));
    }
    //TODO task tek lerin karesini buyukten kucuge
    public static void teklerinKareBuyuktenKucuge(List<Integer> sayi) {

        sayi.stream().filter(Methodlarim::tekelamaniBul)
                .map(t->t*t)//her sayinin karesi alindi
                .sorted(Comparator.reverseOrder())//buyukten kucuge
                .forEach(Methodlarim::yazdir);//yazdirdik
    ...
    public static void alfabetikBuyukHarfTekrarsiz(List<String> yemek) {
        yemek.stream().//
                ...
                .comparing(t-> t.toString()//her bir eleman strige cevrildi
...
yemek.stream().distinct().map(t->t.length()).sorted(Comparator.comparing(t->t.toString()).reversed()).forEach(Utils::ayniSatirdaBosluklaYazdir);
...
//findfirst ile coz
//Stream<String>sonisim=yemek.stream().sorted(Comparator.comparing(t->t.toString().length()).reversed()).limit(1);//bu bize stream clasondan bir elemant donduruyor bur
